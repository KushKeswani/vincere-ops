...
Quit also binds one authoritative `process_*` reference, a canonical process-state version, and a runtime digest observed within 30 seconds. Its one-use interactive approval and the command both expire within 60 seconds. The runtime summary must prove:
...
The companion must repeat the preflight immediately before actuation and block on any changed digest or target. Completion binds exactly one process observation and requires a fresh authoritative stopped-state version and observation time. An exact already-stopped process tombstone returns the verified `ALREADY_STOPPED` no-op without invoking process control. The only permitted mutating quit operation is the allowlisted graceful-close mechanism. Force-kill, order cancellation, position flattening, connection disconnection, strategy mutation, implicit retries, and password automation are structurally absent or fixed to `false` in evidence.
...
The server reselects the tenant-scoped agent, requires its advertised `process.control` capability, reads a fresh authenticated process heartbeat, and derives the target and expected process-state version. Graceful quit additionally requires Runtime-v2 evidence no older than 30 seconds with strict `asOf <= event occurredAt <= server receivedAt <= current server time` chronology, overall completeness plus complete process/Add-On/account/strategy/position/order scopes, a healthy connected authenticated Add-On, the exact same running process identity, simulation-only accounts, definitively disabled stable strategies, no positions, no working/transitional/unknown orders, and no queued/delivered/indeterminate process command.
...
When opted in, the companion uses a separate Windows observation adapter for Runtime-v2 collection and injects fresh Runtime-v2 evidence providers into the process-control adapter. Launch readiness requires a fresh complete Add-On scope with a healthy connected authenticated Add-On. Quit runtime state requires a fresh, schema-valid, overall-complete authoritative Runtime-v2 observation; otherwise the provider returns null and graceful quit is blocked. Non-simulation or unknown accounts, enabled/unknown strategies, open positions, and working/transitional/unknown orders remain blockers. The current companion has no scheduler actuator, so its local schedule count is zero; process control must not be enabled alongside a future scheduler until that scheduler supplies its durable armed-schedule authority to this preflight.
