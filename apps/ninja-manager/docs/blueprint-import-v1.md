...
- Commit and approval require server age no greater than 30 seconds, valid `asOf <= occurred <= received <= server now` chronology, consistent declared age, healthy authenticated Add-On IPC, and complete account, connection, and strategy scopes.
- Every mapped account must be authoritatively classified by NinjaTrader as simulation, connected through complete healthy known connection evidence, and unique in the revision. Every normalized strategy/instrument row must match exactly one stable current strategy instance on that account. One runtime strategy instance may be bound only once per revision.
...
CSV parity, schedules, and all control commands remain deferred. Those require their own authorization, audit, and runtime-actuation review.
