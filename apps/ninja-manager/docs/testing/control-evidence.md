...
- the full CENTRAL_CONNECTED project remains red because an older client journey still tries to operate the removed `Primary objective` questionnaire on the current disabled Blueprint import page. That unrelated test/product mismatch is not counted as runtime evidence;
